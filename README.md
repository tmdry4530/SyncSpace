...
- 서버 목록(채널/문서/메시지 히스토리) 변경은 TanStack Query 폴링으로 반영 (실시간 채팅·문서·presence는 Yjs WebSocket)
...
| Backend | Node.js HTTP + WebSocket 서버 (REST API + Yjs WS + SPA 정적 서빙) |
| Database/Auth | Railway Postgres, 앱 세션 인증 (scrypt + HttpOnly 쿠키) |
| Deploy | Railway (단일 서비스 + Postgres) |
...
├───────────────────────────────────────────────────────────┤
│ Node Backend          SPA 서빙 + REST API + WebSocket upgrade │
├───────────────────────────────────────────────────────────┤
│ Railway Postgres      세션 인증 + 데이터 + Yjs 스냅샷         │
└───────────────────────────────────────────────────────────┘
```
...
로컬에서 인증된 앱 흐름을 확인하려면 Postgres + 세션 인증 env가 필요합니다.

```txt
DATABASE_URL=postgresql://...        # 로컬 또는 Railway Postgres
AUTH_SECRET=<랜덤 시크릿>            # 세션 토큰/IP 해시 서명 (WS_AUTH_MODE=session 필수)
WS_AUTH_MODE=session
VITE_API_URL=http://localhost:1234
VITE_WS_URL=ws://localhost:1234
VITE_WS_AUTH_MODE=session
```

`AUTH_SECRET`·`DATABASE_URL`는 backend 전용입니다. `VITE_` 환경 변수에 넣지 않습니다.
