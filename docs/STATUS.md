...
GStack startup idea review 완료 on 2026-05-03: SyncSpace를 포트폴리오가 아닌 창업 아이디어로 평가하고 `decision memory` 포지셔닝을 제안.
...
- [x] GStack startup idea review 완료: `docs/reports/GSTACK_STARTUP_IDEA_REVIEW_2026-05-03.md`
...
- GStack startup idea review evidence:
  - report: `docs/reports/GSTACK_STARTUP_IDEA_REVIEW_2026-05-03.md`
  - gstack review log/read completed at commit `fe6079f` with `status=completed`, `unresolved=0`, `critical_gaps=2`
...
## Git Exclusion Update — 2026-05-03

- Updated `.gitignore` so Git ignores non-README Markdown files, test scripts/results, and local AI/agent artifacts.
- Removed already-tracked excluded files from the Git index with `git rm --cached`; files remain in the working tree.
- Validation evidence:
  - `git ls-files '*.md'` now lists only README files.
  - tracked test/AI pattern checks return no matches.
  - `git check-ignore -v` confirms representative excluded files are ignored and README files are not ignored.

## Repository Warning Cleanup — 2026-05-03

- Removed tracked script/config references that pointed at now-ignored test artifacts.
- Updated backend CI to validate lint/typecheck/build instead of running excluded tests.
- Removed Playwright/Vitest package references from published package manifests.
- Validation evidence:
  - `pnpm typecheck` passed.
  - `pnpm verify:all` passed.
  - tracked reference scan found no Playwright/Vitest/test-artifact references in published configs.

## React Code Reading Guide — 2026-05-03

- Added a root README section explaining how to read the React project by entrypoint, router, protected app providers, workspace layout, state/data hooks, and realtime hooks.
- Updated public README references to match the current repository policy: docs/tests/AI artifacts are local-only, root README is the tracked documentation surface.
- Validation evidence:
  - README stale reference scan found no `docs/`, Playwright/Vitest, or removed test command references.

## Separate React Code Guide — 2026-05-03

- Split the React code reading guide out of README into `REACT_CODE_GUIDE.md`.
- Kept README as a short link to the separate guide.
- Added `.gitignore` exception so this one explicit learning guide is tracked while other non-README Markdown remains ignored.

## README Refresh — 2026-05-07

- Root `README.md`를 리뷰어 온보딩 흐름에 맞춰 재구성했다.
- 프로젝트 가치 제안, 상태 책임 분리, realtime/data flow, 실행/배포/검증 명령, 문서 길잡이를 한 문서에서 빠르게 파악할 수 있게 정리했다.
- Frontend protected core files는 수정하지 않았다.
- Validation evidence:
  - `git diff --check README.md` passed.
  - `python` markdown whitespace/link sanity check for `README.md` and `docs/STATUS.md` passed.

## Presentation Demo URL — 2026-05-07

- Root `README.md`에 발표용 live deployment URL을 추가했다: `https://sync-space-green.vercel.app/`.
- 발표 데모 동선과 `/auth/login`, `/api-contract` 빠른 링크를 README 상단에 추가했다.
- Validation evidence:
  - `curl -I -L https://sync-space-green.vercel.app/` returned HTTP 200 from Vercel.
  - `curl -I -L https://sync-space-green.vercel.app/auth/login` returned HTTP 200 from Vercel SPA rewrite.
  - `curl -I -L https://sync-space-green.vercel.app/api-contract` returned HTTP 200 from Vercel SPA rewrite.

## README Walkthrough Presentation Script — 2026-05-07

- Added `docs/PRESENTATION_SCRIPT_README_WALKTHROUGH.md` for PPT-less README walkthrough presentation.
- Linked the script from root `README.md` document guide.
- Frontend protected core files were not modified.
- Validation evidence:
  - `git diff --check README.md docs/PRESENTATION_SCRIPT_README_WALKTHROUGH.md` passed.
  - Markdown sanity check passed for the new presentation script link and live demo URL.

## README-based Presentation Q&A — 2026-05-07

- Added `docs/PRESENTATION_QA_README_BASED.md` with README-based expected questions and model answers.
- Linked the Q&A document from root `README.md` document guide.
- Frontend protected core files were not modified.
- Validation evidence:
  - `git diff --check README.md docs/PRESENTATION_QA_README_BASED.md` passed.
  - Markdown sanity check passed for Q&A headings, live demo URL, and README link.

## README Visual Structure Refresh — 2026-05-07

- Reworked root `README.md` to reduce text-only presentation density.
- Added visual hierarchy with badges, summary cards, tables, Mermaid flowcharts, sequence diagram, mindmap, and deployment diagrams.
- Preserved live demo URL, presentation links, architecture contracts, setup, deployment, and verification guidance.
- Frontend protected core files were not modified.
- Validation evidence:
  - `git diff --check README.md` passed.
  - README sanity check passed for Mermaid fences, live demo URL, and presentation document links.

## README Visual Refresh Reverted — 2026-05-07

- Reverted root `README.md` from the badge/Mermaid-heavy visual version back to the previous text-first presentation/README walkthrough structure.
- Preserved the live demo URLs, presentation script link, Q&A link, setup, deployment, and verification sections.
- Frontend protected core files were not modified.
- Validation evidence:
  - `git diff --check README.md` passed.
  - README sanity check passed for no Mermaid fences, live demo URL, and presentation document links.

## README Project-only Rewrite — 2026-05-07

- Rewrote root `README.md` to remove presentation-specific sections and links.
- README now focuses on project identity, live URL, feature set, technology stack, architecture, core logic, security, setup, backend API, deployment, and verification.
- Removed README links to presentation script and Q&A documents while keeping those docs in `docs/`.
- Frontend protected core files were not modified.
- Validation evidence:
  - `git diff --check README.md` passed.
  - README sanity check passed for live URL, core technology keywords, no presentation script/Q&A links, and balanced code fences.

## README React-focused Rewrite — 2026-05-07

- Rewrote root `README.md` around the React frontend portfolio surface.
- Expanded frontend architecture, route/provider structure, feature folders, Zustand/TanStack Query/Yjs state separation, workbench, chat panel, editor, presence, and revalidation logic.
- Reduced backend content to a support-role summary while preserving security/deployment context.
- Frontend protected core files were not modified.
- Validation evidence:
  - `git diff --check README.md` passed.
  - README sanity check passed for React-focused keywords, live URL, reduced backend section count, no presentation script/Q&A links, and balanced code fences.

## README React-focused Compression — 2026-05-07

- Compressed root `README.md` while keeping the React frontend focus.
- Reduced long architecture/setup/backend explanations and kept the core project summary, React structure, state separation, core React logic, realtime rooms, short backend support summary, validation, deployment, and docs links.
- Frontend protected core files were not modified.
- Validation evidence:
  - `git diff --check README.md` passed.
  - README sanity check passed for React-focused sections, live URL, reduced length, no presentation script/Q&A links, and balanced code fences.

## Presentation Script Synced to Compressed README — 2026-05-07

- Updated `docs/PRESENTATION_SCRIPT_README_WALKTHROUGH.md` to match the current compressed React-focused `README.md`.
- Script now follows Live, features, stack, React structure, state separation, Workspace/Chat/Editor/Presence logic, realtime room rules, short backend support, execution, validation, and deployment sections.
- Root `README.md` was not changed for presentation content.
- Validation evidence:
  - `git diff --check docs/PRESENTATION_SCRIPT_README_WALKTHROUGH.md` passed.
  - Script sanity check passed for README-aligned section names, live URL, React-focused keywords, and balanced code fences.
