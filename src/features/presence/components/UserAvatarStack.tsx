...
export function UserAvatarStack({ states }: { states: AwarenessState[] }) {
  ...
  return (
    <div className="avatar-stack ap-wb-avatars" aria-label="현재 접속자">
      ...
      {uniqueUsers.length > 5 ? <span className="avatar-extra ap-wb-avatar-extra">+{uniqueUsers.length - 5}</span> : null}
    </div>
  )
}
